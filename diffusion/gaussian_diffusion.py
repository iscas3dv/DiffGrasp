...
class GaussianDiffusion:
    ...
    def p_mean_variance(
        self, model, x, t, batch, diff_optim, clip_denoised=True, denoised_fn=None, model_kwargs=None
    ):
        """
        ...
        ### GUIDANCE(refinement) here       
        opt_t = [50]
        if t[0] in opt_t:
            opt_result = diff_optim.fitting(batch=batch, net_output=model_output, t=t[0])
        elif t[0]>=0 and t[0]<50:
            opt_result = diff_optim.load_opt_result()
            refined_result = opt_result['img']
            weight_model = (t+1)/1000
            model_output =  weight_model * model_output + (1-weight_model) * refined_result
        ...
        if self.model_var_type in [ModelVarType.LEARNED, ModelVarType.LEARNED_RANGE]:
            ...
            model_variance, model_log_variance = {
                ...
                ModelVarType.FIXED_SMALL: ( ### in here
                    self.posterior_variance,
        ...
        def process_xstart(x):
            if denoised_fn is not None:
                x = denoised_fn(x, t, model_kwargs)
            if clip_denoised:
        ...
        if self.model_mean_type == ModelMeanType.PREVIOUS_X:
            ...
            if self.model_mean_type == ModelMeanType.START_X: ### in here
    ...
    def training_losses(self, model, x_start, t, model_kwargs=None, noise=None, dataset=None):
        ...
        # else:
        #     mask = torch.ones((x_start.shape[0], 1, 1, x_start.shape[3]), dtype=torch.bool, device=x_start.device) 
        ...
        if 'inpainting_mask' in model_kwargs['y'].keys() and 'inpainted_motion' in model_kwargs['y'].keys():
            inpainting_mask, inpainted_motion = model_kwargs['y']['inpainting_mask'], model_kwargs['y']['inpainted_motion']
            assert self.model_mean_type == ModelMeanType.START_X, 'This feature supports only X_start pred for now!'
            assert x_t.shape == inpainting_mask.shape == inpainted_motion.shape
            x_t = (x_t * ~inpainting_mask) + (inpainted_motion * inpainting_mask)
        ...
        if self.loss_type == LossType.KL or self.loss_type == LossType.RESCALED_KL:
            ...
            #     terms["vel_mse"] = self.masked_l2(target_vel, # Remove last joint, is the root location!
            #                                       model_output_vel,
            #                                       mask[:, :, :, 1:])  # mean_flat((target_vel - model_output_vel) ** 2)
