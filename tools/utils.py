...
def rotation_matrix_to_euler_yzx(rot_mats):
    """
    将旋转矩阵转换为欧拉角 (YZX 顺序)
    ...
    """
    assert rot_mats.shape[-2:] == (3, 3), "输入的最后两个维度必须是 (3, 3)"
    
    sy = -rot_mats[..., 2, 1]
    cy = torch.sqrt(rot_mats[..., 2, 0] ** 2 + rot_mats[..., 2, 2] ** 2)
    
    mask = cy > 1e-6
    
    # 计算欧拉角
    y = torch.where(mask, torch.atan2(sy, cy), torch.atan2(sy, torch.tensor(1.0, device=rot_mats.device)))
    z = torch.where(mask, torch.atan2(rot_mats[..., 1, 0], rot_mats[..., 0, 0]), torch.atan2(-rot_mats[..., 1, 2], rot_mats[..., 1, 1]))
    x = torch.where(mask, torch.atan2(rot_mats[..., 2, 0], rot_mats[..., 2, 2]), torch.tensor(0.0, device=rot_mats.device))
    
    return torch.stack((y, z, x), dim=-1)

def euler_yzx_to_rotation_matrix(euler_angles):
    """
    将 YZX 顺序的欧拉角转换为旋转矩阵
    ...
    """
    assert euler_angles.shape[-1] == 3, "输入的最后一个维度必须是 3 (欧拉角)"
    ...
    rot_y = torch.stack([
        ...
        torch.stack([sin_y, torch.zeros_like(y), cos_y], dim=-1)
    ], dim=-2)
    
    # 计算 Z 轴旋转矩阵
    rot_z = torch.stack([
        torch.stack([cos_z, sin_z, torch.zeros_like(z)], dim=-1),
        torch.stack([-sin_z, cos_z, torch.zeros_like(z)], dim=-1),
    ...
    rot_x = torch.stack([
        ...
        torch.stack([torch.zeros_like(x), cos_x, sin_x], dim=-1),
        torch.stack([torch.zeros_like(x), -sin_x, cos_x], dim=-1)
    ], dim=-2)
    
    # 组合旋转矩阵
    rot_mats = torch.matmul(rot_y, torch.matmul(rot_z, rot_x))
