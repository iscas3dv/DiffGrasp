...
import os
# import mano
from data.process_GRAB_AAAI_v2 import tensor_destandardization, tensor_standardization
...
class DiffOptim(nn.Module):
    ...
    def __init__(self,
        ...
        self.smplx_left_mask = torch.load('smplx_left_mask_bool_list.pt')
        self.smplx_right_mask = torch.load('smplx_right_mask_bool_list.pt')
    ...
    def recalcu_wrist_joints(self, batch, lr2omass):
        ...
        #     mesh_obj.write_obj('/home/guest/hq/hoi/seqs_refine_mesh/mesh_obj_'+'{:05d}'.format(frame_idx)+'.obj')
        ...
        #     bmesh_rj.write_obj('/home/guest/hq/hoi/seqs_refine_mesh/sphere_rj_'+'{:05d}'.format(frame_idx)+'.obj')
        ...
        #     amesh_rj.write_obj('/home/guest/hq/hoi/seqs_refine_mesh/sphere_rj_refine_'+'{:05d}'.format(frame_idx)+'.obj')
        #     sp_anim.add_frame([mesh_obj,bmesh_lj,bmesh_rj,amesh_lj,amesh_rj], 
        #                         ['mesh_obj', 'bmesh_lj','bmesh_rj','amesh_lj','amesh_rj'])
        # animation_path = os.path.join('/home/guest/hq/hoi/seqs_refine_mesh')
    ...
    def fitting(self, batch, net_output, t):
        with torch.enable_grad():
            ...
            # 先反归一化出来
            # for k,v in net_outdict.items():
            #     print(k,v.shape)
            # for k,v in norm_pose.items():
            #     print(k,v.shape)
            # exit()
            ...
            opt_params_joints = {'smplx_transl2world': nn.Parameter(net_outdict['smplx_transl2world'].reshape(30,3)), 
            ...
            # opt_joints_consis = optim.Adam([upper_body_rotmat,opt_params_joints['smplx_transl2world']], lr=lr_joints_consis)
            ...
            for itr in range(num_iters_joints):
                ...
                loss_total, losses, contact_label = self.calc_joints_consis_loss(batch, recon_results, opt_params_joints['lr2omass'])
                ...
                # upper_body_rotmat[:,[i-12 for i in left_arms],...].grad = contact_label[:,0].reshape(30,1,1,1) * opt_params_joints['pose_rotmat'].grad[:,left_arms,...]
                # upper_body_rotmat[:,[i-12 for i in right_arms],...].grad = contact_label[:,1].reshape(30,1,1,1)  * opt_params_joints['pose_rotmat'].grad[:,right_arms,...]
                # print(type(opt_params_joints['pose_rotmat'].grad))
                # print(opt_params_joints['pose_rotmat'].grad.shape)
                # print(opt_params_joints['pose_rotmat'].grad.dtype)
                # exit()
                opt_grad = torch.zeros_like(upper_body_rotmat).to(self.device).to(torch.float32)
                opt_grad[:,[i-12 for i in left_arms],...] = contact_label[:,0].reshape(30,1,1,1) * opt_params_joints['pose_rotmat'].grad[:,left_arms,...]
                opt_grad[:,[i-12 for i in right_arms],...] = contact_label[:,1].reshape(30,1,1,1) * opt_params_joints['pose_rotmat'].grad[:,right_arms,...]
            ...
            net_outdict['smplx_transl2world'] = opt_params_joints['smplx_transl2world'].detach().clone()
            ...
            if t==50:
                ...
                opt_grd_contact = optim.Adam([opt_params_gnd[k] for k in ['smplx_transl2world']], lr=lr_grd_contact)
                num_iters_grn = 50
            ...
            #     after_mesh.write_obj('/home/guest/hq/hoi/seqs_refine_mesh/after_refine_'+'{:05d}'.format(i)+'.obj')
            #     before_mesh = Mesh(v=to_cpu(before_verts[i]), f=smplx_face, vc=name_to_rgb['green'])
            #     before_mesh.write_obj('/home/guest/hq/hoi/seqs_refine_mesh/before_refine_'+'{:05d}'.format(i)+'.obj')
    ...
    def dict_destandardization(self, results, smplx_norm_pose, fs=30,):
        ...
        results['smplx_transl2world'] = tensor_destandardization(results['smplx_transl2world'].reshape(fs,3), 
                                                      smplx_norm_pose['transl2world_mean'].to(self.device), smplx_norm_pose['transl2world_std'].to(self.device))
    ...
    def dict_standardization(self, results, smplx_norm_pose, fs=30):
        ...
        results['smplx_transl2world'] = tensor_standardization(results['smplx_transl2world'].reshape(fs,3), 
                                                      smplx_norm_pose['transl2world_mean'].to(self.device), smplx_norm_pose['transl2world_std'].to(self.device)).reshape(30,-1)
